import { PrismaClient, UserRole, OptInStatus, FinancialStatus, FulfillmentStatus, PaymentMethod, TemplateCategory, TemplateStatus, AutomationTrigger } from '@prisma/client';
...
async function main() {
  ...
  for (let i = 1; i <= 5; i++) {
    const customer = await prisma.customer.create({
      data: {
        ...
        firstName: `First${i}`,
        lastName: `Last${i}`,
        whatsappOptIn: OptInStatus.OPTED_IN,
        totalSpend: 1500 * i,
        orderCount: i,
        city: 'Mumbai',
        state: 'Maharashtra',
  ...
  for (let i = 0; i < 10; i++) {
    ...
    await prisma.order.create({
      data: {
        ...
        financialStatus: isCod ? FinancialStatus.PENDING : FinancialStatus.PAID,
  ...
  await prisma.automation.create({
    data: {
      ...
      triggerType: AutomationTrigger.ORDER_CREATED,
      isActive: true,
      flowDefinition: {
